...
import { Container, Typography, TypographyVariant, Background, BackgroundVariant, Section } from '@codezeniths/components';
...
const FEATURES = [
    ...
    { 
        ...
        title1: 'Social and Collaborative Layer',
        desc1: 'The social and collaborative layer of CodeZeniths, creating an active engineering community rather than a traditional forum.',
        desc2: 'Enables professional profiles, showcasing achievements, publishing projects, study groups, discussions, mentoring, and organizing events.',
        title2: 'Unified Developer Profile',
        desc3: 'Surfaces all your platform activity on a single, comprehensive engineering profile.',
        highlights: [
            { title: 'Showcase Achievements', desc: 'Display AlgoWars contest rankings, Intervyn experiences, and published ZenLab projects.' },
            { title: 'Community Collaboration', desc: 'Join organization workspaces, university communities, mentorship programs, and study groups.' },
            { title: 'Knowledge Sharing', desc: 'Seamlessly share CodeFlow visualizations, ZenDraw diagrams, and Archivis articles.' }
        ]
    }
];
...
export const FeatureDetailsSection = () => {
    ...
    return (
        <Section id="feature-details" className="bg-background-light dark:bg-background-dark relative pb-24 pt-18">
            <Container size="5xl" className="mx-auto px-6 lg:px-8">
                ...
                >
                    <button 
                        ...
                        className="flex items-center justify-center w-12 h-12 rounded-xl cursor-pointer bg-foreground-light dark:bg-foreground-dark text-body-light dark:text-body-dark disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-xl"
                    >
                        <ChevronLeft className="w-6 h-6" />
                    </button>

                    <div className="bg-foreground-light dark:bg-foreground-dark rounded-xl border border-white/5 shadow-xl p-2 flex relative w-full items-center justify-center flex-1">
                        <div className="grid grid-cols-3 w-full gap-2">
                            {visibleTabs.map((feature) => (
                                <button
                                    key={feature.id}
                                    onClick={() => setActiveTab(feature.id)}
                                    className={`relative flex items-center justify-center gap-3 px-4 lg:px-6 py-4 cursor-pointer transition-colors rounded-lg w-full hover:bg-primary/10 ${
                                        activeTab === feature.id
                                            ? 'text-body-light-shade1 dark:text-body-dark-shade1'
                                            : 'text-muted-light-shade1 dark:text-muted-dark-shade1'
                                    }`}
                                >
                                    <div className="flex items-center justify-center w-8 h-8 rounded-full border border-current opacity-70 text-sm font-medium shrink-0">
                                        {feature.number}
                                    </div>
                                    <span className="font-semibold text-[14px] lg:text-[15px] truncate">{feature.label}</span>
                                    {activeTab === feature.id && (
                                        <div className="absolute bottom-0 left-4 right-4 h-0.75 bg-primary rounded-t-md" />
                                    )}
                                </button>
                            ))}
                        </div>
                ...
                {/* Tab Content Area */}
                <div className="relative rounded-3xl overflow-hidden shadow-xl mx-0 lg:mx-16">
                    ...
                    />
                        <motion.div 
                            ...
                            viewport={{ once: true, margin: "-100px" }}
                            exit="exit"
                            className="relative z-10 p-8 lg:p-12 flex flex-col gap-24"
                        >
                            {/* Top Section */}
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-18 items-center">
                                ...
                                >
                                <Typography variant={TypographyVariant.H3} className="text-3xl font-bold text-foreground-dark-shade3 dark:text-foreground-light-shade3">
                                    {activeFeature.title1}
                                </Typography>
                                <Typography variant={TypographyVariant.P} className="text-muted-light-shade1 dark:text-muted-dark-shade1 leading-relaxed">
                                    {activeFeature.desc1}
                                </Typography>
                                <Typography variant={TypographyVariant.P} className="text-muted-light-shade1 dark:text-muted-dark-shade1 leading-relaxed">
                                    {activeFeature.desc2}
                                ...
                                <motion.div 
                                    ...
                                    className="relative flex justify-center items-center h-87.5 w-full rounded-2xl bg-muted-light/5 dark:bg-muted-dark/20 p-6 mask-[linear-gradient(to_bottom,white_60%,transparent_100%)]"
                                >
                                {isDark ? (
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-18 items-center">
                                <motion.div 
                                    ...
                                    className="relative flex justify-center items-center h-87.5 w-full rounded-2xl bg-muted-light/5 dark:bg-muted-dark/20 p-6 order-2 lg:order-1 mask-[linear-gradient(to_bottom,white_60%,transparent_100%)]"
                                >
                                {isDark ? (
                                    ...
                                    <div className="flex flex-col gap-4 mt-2">
                                        {activeFeature.highlights.map((highlight, index) => (
                                            <div key={index} className="flex gap-4 items-start">
                                                <div className="flex items-center justify-center w-12 h-12 rounded-full border border-muted-light/20 dark:border-muted-dark/30 bg-muted-light/5 dark:bg-muted-dark/10 font-bold text-lg text-foreground-dark-shade3 dark:text-foreground-light-shade3 shrink-0">
                                                    0{index + 1}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="font-semibold text-lg text-foreground-dark-shade3 dark:text-foreground-light-shade3">{highlight.title}</span>
                                                    <span className="text-sm text-muted-light-shade1 dark:text-muted-dark-shade1 mt-0.5">{highlight.desc}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>

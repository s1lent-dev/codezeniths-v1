...
export const TopicSection: React.FC = () => {
        ...
        return (
            <Card className="rounded-md border border-foreground-light-shade3 dark:border-foreground-dark-shade1 bg-foreground-light dark:bg-foreground-dark p-12 text-center my-8">
                ...
                <Typography variant={TypographyVariant.P} className="text-muted-light dark:text-muted-dark text-sm mb-6">
                    {error?.message || `We couldn't find details for topic "${topicSlug}".`}
                </Typography>
                <Link
                    href={moduleSlug ? `/modules/${moduleSlug}` : '/modules'}
    ...
    const currentModuleSlug = topicDetails?.module?.slug || moduleSlug || 'module-dsa';
    const currentModuleTitle = topicDetails?.module?.title || 'Modules';
        ...
        <div className="w-full space-y-6 pb-12">
            ...
            <BreadcrumbHeader
                ...
                items={[
                    ...
                    { label: currentModuleTitle, href: `/modules/${currentModuleSlug}` },
                    { label: topicDetails?.title || 'Topic', isCurrentPage: true },
                ]}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-6 w-full max-w-full min-w-0 items-start">
                ...
                <div className="w-full lg:w-0 lg:flex-1 min-w-0 max-w-full lg:max-w-[calc(100%-354px)] xl:max-w-[calc(100%-384px)] space-y-6">
                    <ProblemsSection pageContext="topic" fixedTopicSlug={topicSlug} />
                </div>
            </div>
        </div>
    );

...
export const SearchResultsDropdown: React.FC<SearchResultsDropdownProps> = ({
  ...
  const isInline = behavior === 'inline';
  ...
  return (
    ...
    >
      <div className={cn(isInline ? 'max-h-60 overflow-y-auto overscroll-contain pr-1 space-y-2' : 'max-h-120 h-[65vh] overflow-y-auto pr-1 space-y-3')}>
        ...
        {isQueryEmpty ? (
          isHistoryLoading ? (
            <div className="flex flex-col items-center justify-center py-8 gap-2.5 text-muted-light dark:text-muted-dark text-center">
              <Spinner variant={SpinnerVariant.LOADER_CIRCLE} className="w-5 h-5 text-primary mx-auto" />
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <div className="size-10 rounded-full bg-secondary/15 text-muted-light dark:text-muted-dark flex items-center justify-center mb-2.5">
                <History className="size-5" />
              </div>
              <Typography
                variant={TypographyVariant.P}
                align={TypographyAlign.CENTER}
                className="text-xs sm:text-sm font-semibold text-heading-light dark:text-heading-dark text-center"
              >
                No recent searches
              </Typography>
              <p className="text-[11px] text-muted-light dark:text-muted-dark mt-1 max-w-xs text-center">
              ...
              <div className="flex items-center justify-between px-1 pb-1.5 border-b border-foreground-light-shade3/40 dark:border-foreground-dark-shade3/40">
                ...
                {onClearHistory && (
                  <button
                    ...
                    className="flex items-center gap-1 text-[10px] sm:text-[11px] font-medium text-muted-light dark:text-muted-dark hover:text-destructive transition-colors cursor-pointer p-0.5 rounded"
                  >
